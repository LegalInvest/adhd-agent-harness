...
2. Message schemas between agents are typically concise JSON packets containing task IDs, tool descriptions, input payloads, and result metadata.
3. Decomposition considers tool registry inspection, input/output contract formation, format constraints, and, in some systems, explicit verification predicates and behavioral checks for each node in the decomposition graph (Xu et al., 24 May 2026).
4. - Parses the system prompt and loads available tool endpoints.
5. This design robustly accommodates new tools—each tool is added to a central registry and automatically becomes available to the planner's decomposition logic, requiring no code changes in executor instantiation (Wang et al., 18 Sep 2025).
...
*本文是「ADHD × AI」系列的第 68 篇，内容基于全网最新情报与研究自动整合生成，并持续迭代更新。*
