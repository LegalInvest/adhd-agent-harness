...
def generate_article_llm(topic: dict, index: int, wiki_retriever, llm, case_retriever=None) -> dict:
    ...
    user = (
        ...
        "写作要求：\n"
        "- **本文只解决一个问题**，推理骨架按问题驱动式五型问题推进（可隐式融入行文）：收敛型（把大问题压成一个可回答的具体问题）→穿透式（双域机制拆解+谁在获利+暗面风险）→验证式（证据分级/证伪/反例/N-of-1）→决策式（做什么/不做什么/先做什么）→落地式（第一步/最小动作/门槛）；\n"
        "- 1800-2500 字，markdown，用 ## 小节组织；**开头必须是一个具体的冲突/失败现场**（具体的人在具体场景里的具体失败，必须是这个问题独有的，不得复用句式模板），再用同构脊柱给出答案；\n"
